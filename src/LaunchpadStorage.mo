...
actor LaunchpadStorage {
  ...
  public shared func addTransaction(managerAddress: Text, trans: Launchpad.HistoryTransaction): async () {
  ...
  public query func getHistoryTransactionsByPage(managerAddress: Text, offset: Nat, limit: Nat) : async CommonModel.ResponseResult<CommonModel.Page<Launchpad.HistoryTransaction>> {
    let transactions: [Launchpad.HistoryTransaction] = Option.get<[Launchpad.HistoryTransaction]>(transactionMap.get(managerAddress), []);
    let result: [Launchpad.HistoryTransaction] = ListUtil.arrayRange<Launchpad.HistoryTransaction>(
      ListUtil.sort<Launchpad.HistoryTransaction>(transactions, func (a: Launchpad.HistoryTransaction, b: Launchpad.HistoryTransaction): Order.Order {
        if (a.time > b.time) { #greater }
        else if (a.time == b.time) { #equal }
        else { #less }
      }),
      offset,
      limit
  ...
  public query func getSettledLaunchpadsByPage(offset: Nat, limit: Nat) : async CommonModel.ResponseResult<CommonModel.Page<Launchpad.Property>> {
    let result: [Launchpad.Property] = ListUtil.arrayRange<Launchpad.Property>(
      ListUtil.sort<Launchpad.Property>(settledLaunchpads.toArray(), func (a: Launchpad.Property, b: Launchpad.Property): Order.Order {
        if (a.endDateTime < b.endDateTime) { #greater }
        else if (a.endDateTime == b.endDateTime) { #equal }
        else { #less }
      }),
      offset,
      limit
